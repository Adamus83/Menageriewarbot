...
async function get(s){ try{const r=await sheets.spreadsheets.values.get({spreadsheetId:SHEET,range:`${s}!A2:Z`});return r.data.values||[]}catch{return[]} }
...
bot.onText(/\/start/, async m=>{
  ...
  bot.sendMessage(m.chat.id,'🌍 PILIH KLAN',{reply_markup:{inline_keyboard:Object.entries(CLANS).map(([k,v])=>[{text:v.name,callback_data:'join_'+k}])}});
});
...
bot.on('callback_query', async q=>{
  ...
  // INI YANG DIPERBAIKI - pakai CLANS[clan]
